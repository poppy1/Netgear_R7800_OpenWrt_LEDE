...
sed -i 's/IMG_PREFIX:=$(VERSION_DIST_SANITIZED)/IMG_PREFIX:=Pjoyrom-$(shell date +%Y%m%d)-$(VERSION_DIST_SANITIZED)/g' include/image.mk
...
# 编译者信息
cat >> .config <<EOF
CONFIG_KERNEL_BUILD_USER="Pjoyrom"
CONFIG_KERNEL_BUILD_DOMAIN="GitHub Actions @ poppy1"
EOF
